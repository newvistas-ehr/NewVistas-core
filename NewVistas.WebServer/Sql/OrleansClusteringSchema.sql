-- Orleans SQL Server Clustering Schema
-- Tables and queries required for AdoNet cluster membership (Orleans 8.x).
-- CleanupDefunctSiloEntriesKey is required by Orleans.Clustering.AdoNet 8.2.0
-- but was omitted from the official SQLServer-Clustering.sql release script.
...
-- CleanupDefunctSiloEntriesKey is required by Orleans.Clustering.AdoNet 8.2.0 DLL
-- but was inadvertently omitted from the official SQLServer-Clustering.sql release script.
-- Removes membership rows whose IAmAlive heartbeat is older than the supplied cutoff date,
-- allowing Orleans to recover from crashed silos that never deregistered.
INSERT INTO OrleansQuery(QueryKey, QueryText)
SELECT
