# Dispensing, Counseling, and Labels -- Pharmacist Human Test Script
...
- **Pre-conditions:**
  ...
  3. Load demo data: enter Patient ID `4` and use the API: `POST /api/outpatientpharmacy/demo/load?patientId=4`
...
2. Enter Patient ID: `4` and click **Load Prescriptions**.
3. Click on **LISINOPRIL 10MG TAB** to select it.
4. The detail panel opens. Confirm the prescription is ACTIVE and has a FillDate.
5. Click the **Record Dispense** button (visible because FillDate is set).
6. The dispense form fields appear. This form may be inline or overlay. Fill in:
   - NDC: `68180-0598-01`
   - Lot#: `2026A-0329`
   - Pharmacist: `PHARM3`
...
1. Select **LISINOPRIL 10MG TAB** (or any verified, active prescription).
2. Click the **Print Label** button in the action bar.
...
1. Select **LISINOPRIL 10MG TAB** (must be verified first for label generation).
2. Click the **View Label** button (appears after verification).
3. The label content panel opens or a dialog with structured label data.
...
1. Select **METFORMIN 500MG TAB** in the prescription list.
2. Check if the Counsel column shows a flag icon. If not, the CounselingRequired flag is false.
3. Click the **Require Counseling** button (in the action bar).
...
- In the prescription table, the Counsel column now shows the flag icon.
...
1. Select a prescription with CounselingRequired = true (from Scenario 4).
...
1. Select **LISINOPRIL 10MG TAB** (a prescription that has been filled and/or refilled).
2. Scroll down in the detail panel to the **Refill History** section.

### Expected Result

- The Refill History table shows columns: #, Date, Qty, Days, Rx#, Pharmacist.
...
1. Select **ATORVASTATIN 40MG TAB** (filled ~10 days ago with 90-day supply, 3 refills authorized).
...
- Refill Status section displays:
  - **Eligible** (green badge) -- if enough time has passed
...
1. Select **METFORMIN 500MG TAB** (30-day supply, filled recently from demo).
...
- Refill Status section displays:
  ...
  - Reasons list (red text, bulleted):
