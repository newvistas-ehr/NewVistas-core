# Prescription Verification, Fill, and Refill -- Pharmacist Human Test Script
...
- **Pre-conditions:**
  ...
  2. Navigate to `/outpatientpharmacy` in the browser.
  3. Enter Patient ID `4` in the Patient ID field and click **Load Demo** (or use the API: `POST /api/outpatientpharmacy/demo/load?patientId=4`).
...
4. Verify the prescription table loads with columns: Drug, Dosage, Status, Priority, Refills Left, Expires, Verified, Counsel, Provider.
...
7. Click on the AMLODIPINE row to select it. The detail panel opens.
...
- The prescription table refreshes; AMLODIPINE row shows Verified: checkmark.
...
4. Click the **Refill** button.

### Expected Result

- Action message: "Action 'refill' completed successfully."
- Refills Left column decrements by 1.
- In the Refill History table at the bottom of the detail panel, a new row appears:
...
2. Verify and fill the prescription using the API:
   ...
   { "fillDate": null }
   ```
3. Click **Load Prescriptions** and select the PREDNISONE row.
4. The Refill Status section should show: **Not Eligible** (red badge).
5. The Reasons list should include: "No refills remaining."
6. The **Refill** button should be disabled (grayed out).

### Expected Result

- Refill Status shows **Not Eligible** with red badge.
...
- Refill Status: **Not Eligible** (red badge).
...
3. Select it in the UI. The Refill Status should show: **Not Eligible**.
...
1. This scenario requires the Drug Utilization Review workflow. Before attempting to fill a prescription, perform a DUR that fails:
   - Navigate to `/drug-utilization-review`.
   - Enter Patient ID: `4`.
   - Click **+ Perform DUR**.
...
3. Return to `/outpatientpharmacy`, select the prescription, and attempt to **Fill**.
...
1. This scenario requires the Interaction Blocking workflow. Before fill:
   - Navigate to `/interaction-blocking`.
   - Enter Patient ID: `4`.
   - Click **+ Screen Rx**.
...
3. Return to `/outpatientpharmacy` and attempt to fill.

### Expected Result

- The fill attempt returns an error indicating the prescription is blocked by a drug interaction.
- The pharmacist must override the interaction in the Interaction Blocking page before fill.
...
1. In the prescription list, click on **ATORVASTATIN 40MG TAB**.
...
5. The detail panel refreshes. Status changes to **HOLD** (orange badge).
...
1. In the prescription list, click on **METFORMIN 500MG TAB**.
2. Click the **Discontinue** button.
3. A text input row appears: "Discontinue reason" with Confirm D/C and Cancel buttons.
...
- The prescription table row shows Status: DISCONTINUED in red.
