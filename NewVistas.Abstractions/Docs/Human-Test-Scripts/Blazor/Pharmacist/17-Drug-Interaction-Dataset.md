# Drug Interaction Dataset Management -- Pharmacist Human Test Script
...
- **Pre-conditions:**
  ...
  2. Navigate to `/drug-interactions` in the browser.
...
2. The **Dataset Status** tab should be active by default.
...
1. On the **Dataset Status** tab, click **Load Demo Dataset**.
2. Wait for the load to complete (this loads sample drug-drug interaction pairs from VistA Files #56-1 through #56-6).

### Expected Result

- A success message appears indicating the demo dataset was loaded.
...
1. Click the **Browse Interactions** tab.
...
- If this pair exists in the dataset, the Interaction Detail panel shows:
  ...
  - Severity: badge showing one of: Minor, Moderate, Significant, Contraindicated
...
- A table of all interaction pairs loaded from the dataset.
...
1. Click the **Check Interactions** tab.
2. This tab provides a way to check multiple ingredients against each other for interactions.
...
4. To clean up, click **Clear Dataset** on the Dataset Status tab.
