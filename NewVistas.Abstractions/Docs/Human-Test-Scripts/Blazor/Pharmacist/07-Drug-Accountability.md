# Drug Accountability -- Inventory Management -- Pharmacist Human Test Script
...
- **Pre-conditions:**
  ...
  2. Navigate to `/drugaccountability` in the browser.
  ...
  5. The drug inventory table should display multiple drugs with balances, reorder points, and stock status.
...
1. Navigate to `/drugaccountability`.
2. Enter Location ID: `VAULT-001` and click **Load Location**.
3. Locate a drug in the inventory table (e.g., the first row).
4. Click the **Receive** button on that drug's row.
...
1. In the inventory table, locate a drug with sufficient balance.
2. Click the **Dispense** button on that drug's row.
...
1. Locate a drug in the inventory table.
2. Click the **Waste** button on that drug's row.
...
1. Record a transfer via the API (the UI does not have a direct transfer panel):
   ```
   ...
   ```
2. Return to the UI and click **Load Location** to refresh.
...
1. Locate a drug in the inventory table. Note its current balance (e.g., 470).
2. Click the **Count** button on that drug's row.
...
1. Record a return via the API:
   ...
   ```
2. Refresh the location in the UI.
...
1. On the inventory table, check the **Low stock only** checkbox.
2. The table filters to show only drugs where balance is at or below the Reorder Point.

### Expected Result

- Only drugs with a **LOW STOCK** badge (red) in the Status column appear.
...
1. On the inventory table, check the **Controlled only** checkbox.
2. The table filters to show only drugs marked as controlled substances.

### Expected Result

- Only drugs with a **DEA** badge in the Type column appear.
