# IV Admixture Compounding Workflow -- Pharmacist Human Test Script
...
- **Pre-conditions:**
  ...
  2. Navigate to `/iv-pharmacy` in the browser.
  3. Enter Patient ID `22` in the Patient ID field.
...
1. Navigate to `/iv-pharmacy`.
2. Enter Patient ID: `22` and click **Load Orders**.
3. Click the **New Order** tab (tab 3).
4. Fill in the new order form:
   ...
   - Start Date/Time: (select current date/time)
   - Stop Date/Time: (select 3 days from now)
   - Provider ID: `PROV-001`
   - Provider Name: `DR. JANE SMITH`
   - Notes: `For hydration and electrolyte replacement`
5. Click **Create Order**.
6. Success banner: "IV admixture order created successfully."
7. Switch to the **Pending / Verified** tab (tab 1). The new order should appear with Status: Pending.
8. Click the **Verify** button on the new order row.
...
12. Switch to the **Compounding Queue** tab (tab 2). The order appears here.
...
- At each step, the appropriate tab shows the order in the correct state.
- The All Orders tab (tab 0) reflects the current status at all times.
...
1. Create a new IV order using the New Order tab (same as Scenario 1 steps 3-5, but with different notes).
...
3. The order appears on the Pending / Verified tab with Status: Pending.
4. Cancel the order via the API (the UI does not have a direct Cancel button on pending orders):
   ```
...
- The order row appears dimmed (row-inactive style) on the All Orders tab.
- The order no longer appears on the Pending / Verified tab.
...
1. Create an IV order via the New Order tab:
...
3. Add additives via the API:
   ...
   ```
4. View the order on the All Orders tab. The Additives column should show 3.

### Expected Result

- The order details show 3 additives when viewed.
- The All Orders table shows Additives column = 3.
...
1. Create a TPN order via the New Order tab:
...
- Additives column shows 6 on the All Orders tab.
...
1. Create a STAT IV order via the New Order tab:
   - Base Solution: `Normal Saline 0.9%`
   - Base Volume (mL): `100`
   - Route: `Peripheral`
   - Frequency: `Once`
...
2. Click **Create Order**.
3. The order appears on the Pending / Verified tab highlighted in red (STAT priority).
...
- The order priority column shows **STAT** in red bold text throughout the workflow.
