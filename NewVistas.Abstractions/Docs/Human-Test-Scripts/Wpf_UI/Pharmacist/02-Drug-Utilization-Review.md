# Drug Utilization Review (DUR) -- Pharmacist Human Test Script -- WPF UI
...
- **Pre-conditions:**
  ...
  2. In the Navigation Panel, select **Drug Utilization Review**.
...
1. In the Navigation Panel, select **Drug Utilization Review**.
2. Enter Patient ID: `9` in the Patient ID field in the toolbar and click **Load**.
3. Click **+ Perform DUR**.
4. Fill in the form:
...
- A success toast notification appears: "DUR assessment completed: DUR-XXXXXXXX"
- The view switches to the Detail tab automatically.
- Assessment status: **Passed** (green status indicator).
- Overall outcome: **Pass**.
- Check Results DataGrid shows all checks with outcome Pass or N/A:
  - Duplicate Drug: Pass
...
- The Detail tab shows status: **Failed** (red status indicator) or **Pending** (yellow status indicator).
- Check Results DataGrid shows:
  - Duplicate Drug: **Fail** (red status indicator)
...
- Check Results DataGrid shows:
  - Drug-Allergy Contraindication: **Fail** (red status indicator)
...
- Check Results DataGrid shows:
  - Days Supply Exceeded: **Fail** (red status indicator)
...
- Check Results DataGrid shows:
  - Controlled Substance: **Warning** (yellow status indicator)
...
- Check Results DataGrid shows:
  - Max Dose Exceeded: **Fail** (red status indicator)
...
1. Load a previously failed DUR assessment from the All or Pending TabItem.
2. Click **View** on a Failed assessment (e.g., the Duplicate Drug from Scenario 2), or right-click and select **View**.
3. The Detail TabItem opens showing the failed check.
4. Locate the failed check row in the DataGrid (e.g., Duplicate Drug with Fail outcome).
5. Click the **Override** button on that row (or right-click and select **Override**).
6. A dialog window appears with fields:
...
- A success toast notification appears: "Override submitted for Duplicate Drug."
- The check row now shows:
  - Outcome: **Override** (gold/brown status indicator)
  - Override column: "Overridden by PHARM1"
  - Hovering or tooltip shows the clinical reason
...
1. On the Detail TabItem of a DUR assessment (either Passed or Overridden), locate the Acknowledge section at the bottom.
2. Enter:
...
- A success toast notification appears: "Assessment acknowledged."
- The assessment status changes to **Acknowledged** (blue status indicator).
- The Acknowledged section in detail shows: date/time and PHARM1.
- The Acknowledge button disappears (already acknowledged).
- On the All TabItem, the assessment row now shows Status: Acknowledged.
- On the Pending TabItem, this assessment no longer appears.
