# Specimen Shipping Manifests -- Pharmacist Human Test Script -- WPF UI
...
- **Pre-conditions:**
  ...
  2. In the Navigation Panel, select **Lab Shipping**.
...
1. In the Navigation Panel, select **Lab Shipping**.
2. Click the **Create Manifest** TabItem.
...
- A success toast notification appears with the new Manifest ID (e.g., "Manifest created: LA-SHIP:XXXXXXXX").
- Switch to the **Manifest Lookup** TabItem.
- Enter the manifest ID and click **Load Manifest**.
- The manifest detail panel shows:
  - Manifest ID
  - Destination: Quest Diagnostics
  - Config ID: QUEST-001
  - Status: **OPEN** (green status indicator)
...
1. With the OPEN manifest loaded, add specimens via the API:
   ...
   ```
2. Reload the manifest in the view by entering the manifest ID and clicking **Load Manifest**.

### Expected Result

- The Specimens section now shows 3 active, 0 removed.
- The specimens DataGrid displays:
  | Specimen ID | Patient | Test | LOINC | Type | Collected | Status | Actions |
  |------------|---------|------|-------|------|-----------|--------|---------|
  | SPEC-001 | 4 | TSH (Thyroid Stimulating Hormone) | 3016-3 | Serum | 03/29 08:00 | **ACTIVE** (green status indicator) | Remove |
  | SPEC-002 | 9 | Vitamin D, 25-Hydroxy | 1989-3 | Serum | 03/29 08:30 | **ACTIVE** (green status indicator) | Remove |
  | SPEC-003 | 16 | HbA1c | 4548-4 | Whole Blood | 03/29 09:00 | **ACTIVE** (green status indicator) | Remove |
...
1. On the loaded manifest, locate SPEC-002 (Vitamin D) in the specimens DataGrid.
2. Click the **Remove** button on the SPEC-002 row (or right-click and select **Remove**).

### Expected Result

- The specimen row now shows:
  - Status: **REMOVED** (red status indicator)
...
1. Ship the manifest via the API:
   ...
   ```
2. Reload the manifest in the view.

### Expected Result

- The manifest detail shows:
  - Status: **SHIPPED** (blue or purple status indicator)
...
- The manifest detail shows:
  - Status: **RECEIVED** (green status indicator)
...
2. Cancel it via the API before shipping:
   ...
   ```
3. Reload the manifest in the view.

### Expected Result

- The manifest detail shows:
  - Status: **CANCELLED** (red status indicator)
...
1. Click the **Shipping Configs** TabItem.
2. The TabItem loads the list of reference lab shipping configurations in the DataGrid.
3. If demo data was loaded, the configs include:
   - Quest Diagnostics
   - LabCorp
   - ARUP Laboratories
4. View a config's details by selecting it in the DataGrid (click or load).
...
- To create or update a config, use the API:
  ...
  ```
- After creating, refresh the configs TabItem. The new config appears in the list.
...
- The shipping configs tie into the Automated Lab Instruments module (select **Lab Instruments** in the Navigation Panel).
