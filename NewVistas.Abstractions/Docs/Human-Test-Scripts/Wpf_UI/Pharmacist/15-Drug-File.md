# Drug File Management -- Pharmacist Human Test Script -- WPF UI
...
- **Pre-conditions:**
  ...
  2. In the Navigation Panel, select **Drug File**.
  3. If the view shows "Drug file data has not been loaded yet", load demo data first (Scenario 6).
...
1. In the Navigation Panel, select **Drug File**.
2. If data is not loaded, click **Load Demo Data** first (see Scenario 6).
3. The **Drugs** TabItem should be active by default.
4. In the search TextBox, type: `METFORMIN`
5. Leave the type ComboBox on "All types".
6. Click **Search**.

### Expected Result

- The drug file results DataGrid populates with matching entries:
  - METFORMIN 500MG TAB
  - METFORMIN 850MG TAB
...
1. From the search results, click on a specific drug in the DataGrid (e.g., METFORMIN 500MG TAB), or right-click and select **View Details**.
...
1. Click the **Routes** TabItem in the TabControl.
2. The view switches to display all available medication routes.

### Expected Result

- A DataGrid of medication routes appears.
...
1. Click the **Dose Units** TabItem in the TabControl.
2. The view switches to display all available dose units.

### Expected Result

- A DataGrid of dose units appears.
...
1. Click the **Orderable Items** TabItem in the TabControl.
2. The view switches to display pharmacy orderable items.
...
3. A busy indicator appears while loading. Wait for the load to complete.
...
4. Use the type ComboBox to filter drugs by dispense type:
