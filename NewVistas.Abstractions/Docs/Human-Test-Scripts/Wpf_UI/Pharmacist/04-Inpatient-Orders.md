# Inpatient Pharmacy Order Verification -- Pharmacist Human Test Script -- WPF UI
...
- **Pre-conditions:**
  ...
  2. In the Navigation Panel, select **Inpatient Pharmacy**.
  3. Enter Patient ID `22` in the Patient ID field in the toolbar and click **Load Demo** (or use the API: `POST /api/inpatientpharmacy/demo/load?patientId=22`).
...
1. Create a new UNIT_DOSE order via the API:
   ...
   ```
2. In the Navigation Panel, select **Inpatient Pharmacy**, enter Patient ID: `22` in the toolbar, and click **Load Orders**.
3. The new VANCOMYCIN order should appear in the DataGrid with:
   - Type status indicator: UD (blue)
   - Status: PENDING (yellow status indicator)
   - Verified: "Pending" (warning icon)
4. Click on the VANCOMYCIN row in the DataGrid to open the detail panel.
5. Confirm the detail shows: Order Type: UNIT_DOSE, Drug: VANCOMYCIN 1G INJ, Status: PENDING, Route: IV, Schedule: Q12H.
6. Click the **Verify (RPh)** button (or right-click and select **Verify**).

### Expected Result

- Action message: "Action 'verify' completed."
- The detail panel refreshes:
  - Status changes to **ACTIVE** (green status indicator).
  - Verified by: RPH-CURRENT on (current date/time).
- In the order DataGrid, the Verified column changes from "Pending" to the checkmark.
...
1. Create a new IV order via the API:
   ...
   ```
2. Load orders in the view. Click on the new KCL order in the DataGrid.
3. The detail panel shows Type: IV (purple status indicator).
4. Click **Verify (RPh)** to verify.
5. After verification (Status: ACTIVE), click the **+ Additive** button.
6. In the additive form (dialog window or inline panel):
...
- The IV Additives DataGrid now shows:
  | Drug | Dose | Unit | Type |
  |------|------|------|------|
...
1. Create an LVP order via the API:
   ...
   ```
2. Load orders. Click on the D5W order in the DataGrid.
3. Confirm: Type status indicator: LVP (teal), Status: PENDING.
4. Click **Verify (RPh)**.
...
- LVP status indicator displays in teal color.
...
2. In the Navigation Panel, select **Inpatient Pharmacy**, enter Patient ID: `22` in the toolbar, click **Load Orders**.
3. Sort or scan the order DataGrid for rows with Verified column showing "Pending".
...
- The order DataGrid refreshes after each verification.
...
1. Select an ACTIVE order in the DataGrid (e.g., a previously verified VANCOMYCIN order).
2. Click the **Discontinue** button (or right-click and select **Discontinue**).
3. A dialog window appears with a "Discontinue reason" TextBox.
...
1. Select an ACTIVE order in the DataGrid (e.g., POTASSIUM CHLORIDE).
2. Click the **Hold** button (or right-click and select **Hold**).
3. Action message: "Action 'hold' completed."
4. Detail panel shows:
   - Status: **HOLD** (orange status indicator)
...
1. Scheduled administration times are set when the order is created or via the API. To test visibility:
   ...
   ```
2. Reload orders in the view. Select the order in the DataGrid.
3. Under "Scheduled Administration Times", time entries should appear.

### Expected Result

- The detail panel shows a "Scheduled Administration Times" section.
- Time entries display: "03/30 08:00", "03/30 20:00", "03/31 08:00".
- Up to 10 times are shown (the view truncates with `.Take(10)`).
