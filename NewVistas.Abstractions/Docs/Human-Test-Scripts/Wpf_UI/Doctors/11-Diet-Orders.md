# Diet Orders (Dietetics) -- Physician Human Test Script -- WPF UI
...
2. In the Navigation Panel, select **Dietetics**
3. Enter Patient ID in the toolbar: `9`
4. Click **Load** (or press Enter)
5. Note any existing diet orders in the Diet Orders TabItem
6. Click the **+ Create Diet Order** button (green)
7. The "Create Diet Order" form appears
8. Fill in:
   - Diet Type *: **REGULAR** (ComboBox; options: REGULAR, CARDIAC, RENAL, DIABETIC, CLEAR LIQUID, FULL LIQUID, NPO)
   - Current Diet: `Regular diet`
   - Texture: **REGULAR** (ComboBox; options: REGULAR, MECHANICAL SOFT, PUREED, GROUND)
   - Fluid Consistency: **THIN** (ComboBox; options: THIN, NECTAR THICK, HONEY THICK, PUDDING THICK)
...
### Expected Result
- A green notification appears in the status bar: "Diet order created."
- Form closes
- Diet Orders DataGrid reloads
- New order appears in the DataGrid:
  - Diet Type: REGULAR
  - Texture: REGULAR
  - Calories: (may show if set)
  - Modifications: "--"
  - Status: status indicator "ACTIVE" (green)
  - Action button: **D/C**
...
4. Click the new CARDIAC diet row to view the **Detail** TabItem
...
### Expected Result
- A green notification appears in the status bar: "Nutrition goals set."
...
### Expected Result
- A green notification appears in the status bar: "Fluid restriction set."
...
6. Fill in:
   - NPO: **Yes**
   - Start Date: Tonight at midnight (select today's date using the DatePicker)
   - End Date: Tomorrow (select next day using the DatePicker)
7. Click **Set NPO**

### Expected Result
- A green notification appears in the status bar: "NPO status set."
...
4. View detail and set texture consistency:
   - Texture: **MECHANICAL_SOFT** (ComboBox; options: REGULAR, MECHANICAL_SOFT, PUREED, LIQUID)
   - Click **Set Texture**
...
### Steps
1. In the Diet Orders DataGrid, locate an ACTIVE diet order
2. Click the **D/C** button (red) on that row (or right-click and select **Discontinue**)

### Expected Result
- A green notification appears in the status bar: "Diet order discontinued."
- Status changes to a red status indicator (not ACTIVE)
...
### Expected Result
- A green notification appears in the status bar: "Nutrition assessment recorded."
...
### Expected Result
- A green notification appears in the status bar: "Meal preferences saved."
...
### Expected Result
- A green notification appears in the status bar: "Tube feeding set."
- Detail shows:
  - Tube Feeding: Yes
  - Formula: Jevity 1.5
  - Rate: 65.0 mL/hr
...
### Expected Result
- If changes have been made, a DataGrid shows: Date, Change, By
