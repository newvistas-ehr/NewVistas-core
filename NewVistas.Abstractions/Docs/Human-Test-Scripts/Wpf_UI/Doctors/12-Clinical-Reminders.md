# Clinical Reminders -- Physician Human Test Script -- WPF UI
...
2. In the Navigation Panel, select **Reminders**
3. Enter Patient ID in the toolbar: `30`
4. Click **Load** (or press Enter)
5. The reminders DataGrid loads

### Expected Result
- If reminders exist, a DataGrid shows with columns:
  - Reminder, Category, Due Date, Status, Actions
- Reminders with Status "DUE" show a yellow status indicator
- Reminders that are completed show a green status indicator
...
6. If a DUE reminder exists, click the **Done** button on that row (or right-click and select **Done**)
7. If no DUE reminders exist, first create one (see Scenario 2), then complete it

### Expected Result
- A green notification appears in the status bar: "Reminder completed."
- The reminder DataGrid reloads
- The completed reminder's Status changes to a green status indicator (no longer "DUE")
...
3. Fill in:
   ...
   - Category: **CHRONIC DISEASE** (ComboBox; options: IMMUNIZATION, PREVENTIVE, SAFETY, SCREENING, CHRONIC DISEASE)
   - Priority: **NORMAL** (ComboBox; options: NORMAL, HIGH, LOW)
   - Frequency: `1Y` (free-text, e.g., "1Y" for yearly, "6M" for every 6 months)
   - Due Date: Today's date (use DatePicker)
4. Click **Create**

### Expected Result
- A green notification appears in the status bar: "Reminder created."
- The form closes
- The reminders DataGrid reloads
- New reminder appears:
  - Reminder: Annual Diabetic Eye Exam
  - Category: CHRONIC DISEASE
  - Due Date: today
  - Status: "DUE" (yellow status indicator)
...
2. Fill in:
   ...
   - Priority: **NORMAL**
   - Frequency: `1Y`
   - Due Date: October 1 of the current year (or next flu season) using the DatePicker
3. Click **Create**
...
2. Fill in:
   ...
   - Priority: **NORMAL**
   - Frequency: `10Y`
   - Due Date: A date calculated based on patient age (e.g., if patient is 50, due now; if 45, due in 5 years) using the DatePicker
3. Click **Create**
...
2. Fill in:
   ...
   - Priority: **HIGH**
   - Frequency: `3M`
   - Due Date: Today's date using the DatePicker
3. Click **Create**

### Expected Result
- Reminder created with Priority: HIGH
- Appears in the reminders DataGrid
...
### Expected Result
- All three reminders appear in the DataGrid
- DUE reminders (today's date or past) show yellow status indicator with Done button
...
### Steps
1. Click **Done** on "Hemoglobin A1c" reminder (or right-click and select **Done**)

### Expected Result
- A success notification appears: "Reminder completed." -- status changes to completed

### Steps (continued)
2. Click **Done** on "Fall Risk Assessment" reminder

### Expected Result
- A success notification appears: "Reminder completed."
...
### Expected Result
- A red error notification appears in the status bar: "Reminder name is required."
