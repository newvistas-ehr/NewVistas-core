# Progress Notes (TIU Clinical Notes) -- Physician Human Test Script -- WPF UI
...
7. Fill in:
   - Document Type: **PROGRESS NOTE** (ComboBox; options: PROGRESS NOTE, DISCHARGE SUMMARY, CONSULT NOTE, SURGICAL NOTE, CRISIS NOTE, ADVANCE DIRECTIVE)
   ...
   - Note Text:
     ...
     ```
8. Click **Save Note** (or press Ctrl+S)

### Expected Result
- A green notification appears in the status bar: "Note created successfully."
- The New Note form closes
- The notes list reloads
- The new note appears in the DataGrid with:
  - Date: today's date and time
  - Type: PROGRESS NOTE
  - Subject: "Follow-up Hypertension"
  - Author: "SMITH,JOHN A"
  - Status: **UNSIGNED** (yellow status indicator)
  - Location: "PRIMARY CARE CLINIC A"

### Steps (continued -- Sign the Note)
9. Click on the note row in the DataGrid to open the detail view
10. The note detail shows:
    - Status indicator: "PROGRESS NOTE" (blue) and "UNSIGNED" (yellow)
    - Subject, Author, Date, Location
    - Full note text in a pre-formatted block
11. Click the **Sign** button (blue)
12. The Electronic Signature dialog window appears
13. Fill in:
    - Signer ID: `DOCTOR1`
    - Electronic Signature Code: `smythVista1`
14. Click the **Sign** button in the dialog window

### Expected Result
- Dialog window closes
- A green notification appears: "Note signed."
...
### Expected Result
- A success notification appears: "Note created successfully."
- Note appears in the DataGrid with Type: DISCHARGE SUMMARY, Status: UNSIGNED
...
### Expected Result
- If the note has a CosignerId set, status becomes **UNCOSIGNED** (orange status indicator)
...
9. In the Navigation Panel, select **Notes**, load patient 9
10. Click the UNCOSIGNED note
11. Click **Cosign**
12. In the dialog window:
    - Signer ID: `DOCTOR1`
    - Electronic Signature Code: `smythVista1`
13. Click **Cosign**

### Expected Result
- A green notification appears: "Note cosigned."
- Status changes to **COMPLETED** (green status indicator)
...
### Steps
1. In the Navigation Panel, select **Notes**, load patient 9
2. Locate a note with status **COMPLETED**
3. Click the note row to view detail
4. Note: The current WPF view may not have an inline "Amend" button in the detail view, but the workflow grain supports `AmendNoteAsync`. If an Amend button is present:
   - Click **Amend**
...
- The note status changes to **AMENDED** (blue status indicator)
...
### Steps
1. On the Notes view with patient 9 loaded
2. Click the **Note History** button (outline style)
3. The Note History panel expands with date filters:
   - From: 90 days ago (default) -- DatePicker
   - To: Today (default) -- DatePicker
   - Max Results: 100 (default)
4. Click **Load History**

### Expected Result
- A results count shows: "[N] note(s) found"
- A DataGrid shows historical notes with columns: Date, Type, Subject, Author, Status, Location
