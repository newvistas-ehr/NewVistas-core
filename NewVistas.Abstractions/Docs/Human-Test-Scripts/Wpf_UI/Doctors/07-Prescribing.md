# Outpatient Prescribing -- Physician Human Test Script -- WPF UI

## Prerequisites
- Login: DOCTOR2 / Password: smythVista1
- Patient: 30
- Pre-conditions: Demo data loaded. Load demo prescriptions first: in the Navigation Panel select **Outpatient Pharmacy**, enter patient 30, click **Load Demo**. SiloHost, WebServer, and WPF Application (NewVistas.WpfClient) are running.
...
3. Enter Patient ID in the toolbar: `30`
4. Click **Load Prescriptions**
5. Review existing prescriptions in the DataGrid (columns: Drug, Dosage, Status, Priority, Refills Left, Expires, Verified, Counsel, Provider)
6. Note: New prescriptions for outpatient pharmacy are typically placed via the Orders view. In the Navigation Panel, select **Orders**
7. Enter Patient ID in the toolbar: `30`, click **Load**
8. Click the **New Order** TabItem
9. Fill in:
   - Order Type: **Pharmacy**
...
### Expected Result
- A green notification appears in the status bar: "Order placed: LISINOPRIL 10MG TAB DAILY"
- Order appears in Active Orders with Type: "Pharmacy", Status: "Pending"

### Steps (continued)
11. Click the **Sign** button on the new order (or right-click and select **Sign**)
...
## Scenario 2: View Prescription Detail on Outpatient Pharmacy View

### Steps
1. In the Navigation Panel, select **Outpatient Pharmacy**
2. Enter Patient ID in the toolbar: `30`
3. Click **Load Prescriptions** (or **Load Demo** if no prescriptions exist)
4. Click on a prescription row in the DataGrid

### Expected Result
- A **detail panel** appears below the DataGrid showing:
  - Drug name with status indicator (ACTIVE green, DISCONTINUED red, HOLD yellow)
...
- **Refill Eligibility** section (if fill date exists):
  - Eligible/Not Eligible status indicator
...
### Steps
1. In the Navigation Panel, select **Orders**
2. Enter Patient ID in the toolbar: `30`, click **Load**
3. Click **New Order** TabItem
4. Fill in:
   - Order Type: **Pharmacy**
...
1. First, document a Penicillin allergy for patient 30:
   - In the Navigation Panel, select **Allergies**, load patient 30
   - Click **Record Allergy** TabItem
   - Allergen: `Penicillin`
   - Allergen Type: **Drug**
...
2. In the Navigation Panel, select **Orders**, load patient 30
3. Click **New Order** TabItem
4. Fill in:
   - Order Type: **Pharmacy**
...
### Steps
1. In the Navigation Panel, select **Outpatient Pharmacy**, load patient 30
2. Click on an ACTIVE prescription row
3. Click the **Discontinue** button (red) (or right-click and select **Discontinue**)
4. A TextBox row appears: "Discontinue reason"
5. Enter: `Medication changed to alternative agent`
6. Click **Confirm D/C**

### Expected Result
- A green notification appears in the status bar: "Prescription discontinued."
...
2. Click **Hold** (orange button) (or right-click and select **Hold**)

### Expected Result
- Status changes to HOLD (yellow status indicator)
- A **Resume** button appears
- Hold reason displays in detail

### Steps (continued)
3. Click **Resume** (or right-click and select **Resume**)

### Expected Result
- Status returns to ACTIVE (green status indicator)
...
2. Click **Verify** (or right-click and select **Verify**)
