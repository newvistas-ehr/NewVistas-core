            ...
            foreach (ZwrRecord labRecord in chemRecords)
            {
                try
                {
                    ...
                    if (patKey != null)
                    {
                        ...
                        await _grainFactory.GetGrain<IPatientHistoryIndexGrain>($"{patKey}:{PatientHistoryDomains.Lab}")
                            .AddEntryAsync(new HistoryRef { ItemId = grainKey, Date = collectionDate });
